...
contract RedemptionManager is Ownable, ReentrancyGuard {
    ...
    function requestRedemption(
        ...
        uint256 netPayout = valueInBase - fee;
    ...
    function _processRedemption(uint256 requestId) internal {
        ...
        if (req.fee > 0) {
            if (req.payoutMethod == PayoutMethod.NativeToken) {
                (bool feeOk, ) = feeRecipient.call{value: req.fee}("");
                require(feeOk, "Fee transfer failed");
            } else {
                uint256 stableFee = (req.fee * 1e18) / tokenPrices[req.tokenAddress];
                IERC20(req.payoutToken).safeTransfer(feeRecipient, stableFee);
            }
        }
