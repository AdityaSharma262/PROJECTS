...
export default function RecoveryScreen() {
  ...
  const handleValidatePhrase = useCallback(async () => {
    ...
    try {
      const { validateMnemonic } = await import('@/wallet/crypto/mnemonic');
      ...
      setPhraseInput(normalized); // store normalized
      setStep('set_new_pin');
    } catch {
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.footer}>
            <Button
              ...
              disabled={phraseInput.trim().split(/\s+/).length < 12 || validating}
            />
            <Button
              title="Back"
              variant="outline"
